...
#define SEISCOMP_COMPONENT WO2SC
...
#include <seiscomp/processing/parameters.h>
...
// >>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>
WebobsToSeisComP::WebobsToSeisComP(int argc, char** argv) :
	...
	_infoOutput = new FileRotatorOutput(Environment::Instance()->
	        logFile("WebobsToSeisComP-processing-info").c_str(), 60 * 60 * 24, 30);
	...
	_errorOutput = new FileRotatorOutput(Environment::Instance()->
	        logFile("WebobsToSeisComP-processing-error").c_str(), 60 * 60 * 24, 30);
...
// >>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>
WebobsToSeisComP::~WebobsToSeisComP() {
...
// >>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>
bool WebobsToSeisComP::initConfiguration() {
	...
	try {
		_port = configGetInt("wo2sc.socket.port");
	} catch ( ... ) {
		SEISCOMP_ERROR("wo2sc.socket.port variable is not set");
		error++;
	}

	try {
		_regionTag = configGetString("wo2sc.region.tag");
	} catch ( ... ) {
		SEISCOMP_ERROR("wo2sc.region.tag variable is not set");
		error++;
	}

	std::vector<std::string> allowedHosts;
	try {
		allowedHosts = configGetStrings("wo2sc.socket.authorizedHosts");
	} catch ( ... ) {
		SEISCOMP_ERROR("wo2sc.socket.authorizedHosts variable is not set");
		error++;
	}
	...
	try {
		_module_id = configGetInt("wo2sc.module.ID");
	} catch ( ... ) {
		SEISCOMP_ERROR("wo2sc.moduleId variable is not set");
		error++;
	}

	try {
		_module_type = configGetInt("wo2sc.module.type");
	} catch ( ... ) {
		SEISCOMP_ERROR("wo2sc.module.type variable is not set");
		error++;
	}

	try {
		_enableXMLArchive = configGetBool("wo2sc.enable.XMLArchive");
	} catch ( ... ) {
		SEISCOMP_ERROR("wo2sc.enable.XMLArchive variable is not set");
		error++;
	}

	try {
		_enableDatabaseStorage = configGetBool("wo2sc.enable.databaseStorage");
	} catch ( ... ) {
		SEISCOMP_ERROR("wo2sc.enable.databaseStorage variable is not set");
		error++;
	}

	try {
		_enableEventDeletion = configGetBool("wo2sc.enable.eventDeletion");
	} catch ( ... ) {
		SEISCOMP_WARNING("wo2sc.enable.eventDeletion variable is not set, FALSE by default");
		_enableEventDeletion = false;
	}

	try {
		if ( _enableEventDeletion ) {
			std::string deletionMethod = configGetString("wo2sc.eventDeletionMethod");
	...
	try {
		_defaultPath = configGetString("wo2sc.defaultPath");
		if ( _defaultPath.size() > 0 ) {
			if ( _defaultPath[_defaultPath.size() - 1] != '/' )
				_defaultPath += '/';
		}
	} catch ( ... ) {
		SEISCOMP_ERROR("wo2sc.defaultPath variable is not net");
		error++;
	}
...
// >>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>
void WebobsToSeisComP::runDataHandler(int* csock) {
	...
	switch ( retCode ) {
		...
		break;
		default:
			SEISCOMP_LOG(_infoChannel, "WebobsToSeisComP OK!");
		break;
	}
}
// <<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<
...
// >>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>
void WebobsToSeisComP::dataHandler(int* csock) {
	_streamThread = new std::thread(std::bind(&WebobsToSeisComP::runDataHandler, this, csock));
}
// <<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<
...
// >>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>
bool WebobsToSeisComP::clientIsAllowed(const char* address) {
...
// >>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>
int WebobsToSeisComP::handleData(int* csock) {
	...
	try {
		...
		SEISCOMP_LOG(_errorChannel, "WebobsToSeisComP failed to save tmp file : %s", e.what());
		return 3;
	}
	...
	// It allows us to verify if this stream is a legit WebobsToSeisComP message
	...
	// If no error reported, then it's a legit WebObs to SeisComP stream
...
// >>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>
bool WebobsToSeisComP::removeEvent(xmlXPathObject* object) {
	...
	if ( _edm & EDM_Delete ) {
		...
		if ( evt && query()->remove(evt.get(), eventID) ) {
			SEISCOMP_LOG(_infoChannel, "Event %s deleted", eventID.c_str());
			_errorLog += "OK";
		}
	}
...
// >>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>
bool WebobsToSeisComP::createOrigin(xmlDoc* doc, xmlXPathObject* object) {
	...
	if ( _enableDatabaseStorage ) {
		...
		pPick->setWaveformID(WaveformStreamID(eventNetwork, eventStation, locCode, chaCode, ""));
		ep->add(pPick.get());
		...
		if ( eventSminusp != "not set" && !eventSminusp.empty() ) {
			...
			sPick->setWaveformID(WaveformStreamID(eventNetwork, eventStation, locCode, chaCode, ""));
			ep->add(sPick.get());
		}
	...
	if ( _enableXMLArchive ) {
		...
		if ( !Util::pathExists(archivePath) ) {
			if ( !Util::createPath(archivePath) )
				SEISCOMP_LOG(_errorChannel, "WebobsToSeisComP failed to create path %s", archivePath.c_str());
		}
...
// >>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>
void WebobsToSeisComP::handleError(int* csock) {
...
// >>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>
double WebobsToSeisComP::stringToDouble(const std::string& str) {
...
// >>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>
int WebobsToSeisComP::stringToInt(const std::string& str) {
